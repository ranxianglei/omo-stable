...
| `OPENCODE_STORAGE_DIR` | Override the OpenCode storage directory (default `<XDG_DATA_HOME or ~/.local/share>/opencode/storage`). Trimmed; empty/whitespace values ignored; relative paths resolved to absolute. Note: OpenCode core has no equivalent variable and follows `XDG_DATA_HOME` — point the core at the same location via `XDG_DATA_HOME` for session tools to see live data. Useful for test isolation, sandboxes, and custom layouts. |
